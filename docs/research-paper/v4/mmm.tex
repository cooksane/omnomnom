...
For this work, we design a user interface for a cooking application. The interface is iteratively improved based on series of detailed observations in user studies. Our interface aims to maintain germane cognitive load and reduce extraneous load through effective recipe presentation. We focus on three main attributes to effective recipe presentation: instructions, ingredients, and materials. 
...
In the course of our study, we found that users had to repeatedly read and scan the entire recipe to identify the step that they were currently on. This was a major issue of the traditional recipe and caused an unnecessary cognitive load to users. We attempted to reduce this extraneous effort and time spent on finding the current step using semantic zoom. Semantic zoom feature has a capability of magnifying (or demagnifying) not only the geometric properties of data, but also the structure and selection of data. This feature interactively emphasized current instruction based on a user feedback (e.g., a click). With this interface, users no longer had to spend time finding their current steps. 
...
Similar to Responsive interface, our final interface allowed users to jump multiple steps to navigate the recipe. According to Fitts's law \cite{fitts}, this feature ultimately reduces time required to move from one target area to the other. When users hover over a step, as shown in \ref{fig:sz_final_1}, a green arrow button provides users a sense of where they were about to select. Comparing Figure \ref{fig:sz_final_1} and Figure \ref{fig:sz_final_2}, notice the location of the green button remains consistent within step changes. We designed a system in a way that users can navigate the interface using up/down keys, scrolling, or clicking. 
...
Based on how often the ingredient appears in the recipe, we visually showed how much of the ingredient should be used in the current step and how much is expected to be used in later steps. The right tab of Figure \ref{fig:sz_final_1} presents ingredient highlighting feature. Notice 1/2 cup broccoli and 1.2 teaspoon of salt are highlighted as active ingredients used in the current instruction emphasized in semantic zoom view. Similar highlighting technique is adopted for emphasizing current instruction and subgoal label. 
...
In this paper we presented our experiences seeking to develop a recipe interface for a novice cooks. Through series of user studies, we developed some understanding on what is important and extraneous information to anovice cook. We investigated features that increase germane load and reduce extraneous cognitive load in recipe design. We implemented and iteratively tested our interactive recipe interface, which focused on curation of recipe instructions and ingredients. We demonstrated the effectiveness of our interactive interface in comparison to commonly used recipe interfaces. We described the rationale and detailed descriptions of the features that have contributed to the success of our final design iteration.

For future work, we would like to gather more quantitative data to acquire statistically meaningful results. For this, we need to establish a better scheme for finding volunteered subjects. For both studies, we faced difficulties in finding volunteers because each experiment took a considerable amount of time and monetary cost. We would like to further improve our interface by exploring the possibility of emulating similar experiments to what we have conducted. For example, the process of cooking can be emulated using simple objects such as colored blocks and boxes instead of actual food. We believe such experiments would not only improve our recipe interface, but also provide knowledge for improvements to instructional material similar to recipes.
