...
*************** Please skip Introduction for now. We will write it at the end****************
...
While Step-by-step interface reduced cognitive load by clearly separating individual steps, it limited users from having a control over the view of overall picture. Semantic view features keeps the benefit of reducing cognitive load in Step-by-step 

In User Study \#2, both Subject 1 and 3 pointed out that clear separation of steps in Step-by-step interface made for easier digestion without worrying too much of overall picture. This effect reduced cognitive load of users but it did so at the expense of losing the control over the view of overall picture. 

users like to plan current actions in light of what is coming up. 

Incorporated native scrolling feature to navigate individual steps within the recipe. 

 The interface only showed current step, which limited cognitive load but it did so at the expense of knowledge. Hence, it led to longest total cook- ing time compared to Control and Responsive interfaces. 

clear separation of steps made for easier digestion and awareness of what he needed to do without worrying too much of overall picture.? He also stated: ?Even though the second interface was also in paragraph format just like the first one, it didn?t have as daunting of a feel since it was just one.?

Talk about what Michelle added. wrt Gojo's paper. What it does. Sub-goal paper labeling [cite]

\subsection{Ingredient Highlighting}

keeping track of ingredients in relation to current step. 

Based on how often the ingredient appears in the recipe, we can visually show how much should be used in the current step and how much is expected to be used in later steps. 


\subsection{Cookware Listing}
Most subjects requested presentation of cookwares along with instructions and ingredients. In this iteration, we incorporated the feature of cookware curation. List of tools are presented on the right top corner of Figure \ref{fig:sz_final_1}.

\section{Conclusions and Future Work}
In this paper we present our experiences seeking to develop a machine assisted recipe interface for novice chefs. Through the series of user study, we identified strengths and weakness of traditional recipe designs and iteratively refined the recipe interface. The focus of our 

we retained existing design choices that worked well and improved xx that yy. The focus of our improvement is on 

Based on a series of user study, we developed an interactive recipe interface that includes 
